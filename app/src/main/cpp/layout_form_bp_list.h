// Автоматическая генерация кода для макета
	...
	auto bplist23((zLinearLayout*)bplist02->attach(new zLinearLayout(styles_default, 0, false), 
			VIEW_MATCH, VIEW_WRAP)); 
	bplist23->attach(new zViewText(styles_default, 0, R.string.bp_value), 
			VIEW_MATCH, VIEW_WRAP);
	bplist23->attach(new zViewText(styles_default, 0, R.string.bp_mask), 
			VIEW_MATCH, VIEW_WRAP);
	auto bplist33((zLinearLayout*)bplist02->attach(new zLinearLayout(styles_default, 0, false), 
			VIEW_MATCH, VIEW_WRAP)); 
	bplist33->attach(new zViewEdit(styles_bpeditrange1, R.id.bpEditValue, 0), 
			VIEW_MATCH, VIEW_WRAP);
	bplist33->attach(new zViewEdit(styles_bpeditrange1, R.id.bpEditMask, 0), 
			VIEW_MATCH, VIEW_WRAP);
	bplist02->attach(new zViewText(styles_default, 0, R.string.bp_condition), 
			VIEW_WRAP, VIEW_WRAP);
	auto bplist43((zLinearLayout*)bplist02->attach(new zLinearLayout(styles_default, 0, false), 
			VIEW_MATCH, VIEW_WRAP)); 
	bplist43->attach(new zViewSelect(styles_bpspinops, R.id.bpSpinOps), 
			VIEW_MATCH, VIEW_WRAP);
	bplist43->attach(new zViewSelect(styles_bpspincond, R.id.bpSpinCond), 
			VIEW_WRAP, VIEW_WRAP);
	auto bplist53((zLinearLayout*)bplist02->attach(new zLinearLayout(styles_default, 0, false), 
			VIEW_MATCH, VIEW_WRAP)); 
	bplist53->attach(new zViewText(styles_zviewtext1, 0, R.string.bp_address1), 
			VIEW_MATCH, VIEW_WRAP);
	bplist53->attach(new zViewText(styles_zviewtext1, 0, R.string.bp_address2), 
			VIEW_MATCH, VIEW_WRAP);
	bplist53->attach(new zViewText(styles_zviewtext1, 0, R.string.bp_value), 
			VIEW_MATCH, VIEW_WRAP);
	bplist53->attach(new zViewText(styles_zviewtext1, 0, R.string.bp_mask), 
			VIEW_MATCH, VIEW_WRAP);
	bplist53->attach(new zViewText(styles_zviewtext1, 0, R.string.bp_condition), 
			VIEW_MATCH, VIEW_WRAP);
	bplist53->attach(new zViewText(styles_zviewtext1, 0, R.string.bp_access), 
			VIEW_MATCH, VIEW_WRAP);
